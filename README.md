...
4. **Supabase Edge Function `diet-generate`** (generowanie AI — omija limit ~10 s Vercel Hobby):
   ...
   - Zalecane: **`SUPABASE_SERVICE_ROLE_KEY`** (ten sam co *service_role* w API) — funkcja zapisuje plan w tle po długim wywołaniu OpenAI; bez tego używa JWT sesji (zwykle OK, przy bardzo długich jobach JWT może wygasnąć).
   - Bez `OPENAI_API_KEY` plan zostaje w stanie `pending`, a w logach funkcji widać błąd braku klucza.
