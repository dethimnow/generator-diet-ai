...
4. **Supabase Edge Function `diet-generate`** (generowanie AI — omija limit ~10 s Vercel Hobby):
   ...
   - Zalecane: sekret **`SERVICE_ROLE_KEY`** = JWT *service_role* z **Settings → API** (w Edge **nie wolno** nazw z prefiksem `SUPABASE_`). Dzięki temu zapis planu w tle nie zależy od wygaśnięcia JWT użytkownika.
